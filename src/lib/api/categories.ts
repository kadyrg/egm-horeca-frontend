...
export async function getCategoriesAdmin({ page }:{ page: number }) {
  const res = await fetch(`${process.env.ADMIN_API_URL}/categories/?page=${page}`, {
    method: "GET",
...
export async function getCategoryProducts({
  ...
  const locale = await getLocale()
  const isDev = process.env.NODE_ENV === "development";
  const res = await fetch(`${process.env.CLIENT_API_URL}/categories/${slug}/products?page=${page}`, {
    cache: isDev ? 'no-cache' : 'force-cache',
    headers: { 'Accept-Language': locale }
  });
