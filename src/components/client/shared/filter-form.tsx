...
import { Form, FormControl, FormField, FormItem, FormLabel, FormMessage } from "@/components/client/ui/form"
...
function FilterForm() {
  ...
  const minMaxPrice = useSelector((state: RootState) => state.minMaxPrice.minmax);
  ...
  const defMinRange = Boolean(Number(minRange)) ? Number(minRange) : minMaxPrice[0];
  const defMaxRange = Boolean(Number(maxRange)) ? Number(maxRange) : minMaxPrice[1];
