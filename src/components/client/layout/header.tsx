...
import { HeaderMetadata } from "@/lib/types/types";
...
async function Header({ metadata } : { metadata: HeaderMetadata }) {
  return (
    <header className="z-50 w-full sticky top-0 flex flex-col gap-[6px] items-center bg-background shadow-xs px-3 py-3 md:py-[10px] overflow-hidden">
...
async function Categories() {
  ...
  return (
    <div className="hidden md:flex w-full max-w-7xl gap-10">
      {data.map((item) => (
        <Link
          key={item.id}
